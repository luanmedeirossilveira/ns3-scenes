...
RunTest(uint32_t packetSize, double bandwidth, uint32_t nPackets, uint32_t simTime)
{
    ...
    // Criando nós
    NodeContainer nodes;
    nodes.Create(3); // 3 nós: Cliente, Roteador e Servidor

    // Instalando dispositivos de rede entre Cliente e Roteador
    NetDeviceContainer devicesCR = p2p.Install(nodes.Get(0), nodes.Get(1));

    // Instalando dispositivos de rede entre Roteador e Servidor
    NetDeviceContainer devicesRS = p2p.Install(nodes.Get(1), nodes.Get(2));

    // Instalando protocolo de internet
    InternetStackHelper stack;
    stack.Install(nodes);

    // Atribuindo endereços IP entre Cliente e Roteador
    Ipv4AddressHelper addressCR;
    addressCR.SetBase("10.1.1.0", "255.255.255.0");
    Ipv4InterfaceContainer interfacesCR = addressCR.Assign(devicesCR);

    // Atribuindo endereços IP entre Roteador e Servidor
    Ipv4AddressHelper addressRS;
    addressRS.SetBase("10.1.2.0", "255.255.255.0");
    Ipv4InterfaceContainer interfacesRS = addressRS.Assign(devicesRS);
    ...
    // Criando cliente
    Address remoteAddress(InetSocketAddress(interfacesRS.GetAddress(1),
                                            port)); // Usando o endereço IP do nó remoto (Servidor)
...
SaveResultsToFile(const std::string& filename, const std::vector<TestResult>& results)
{
    ...
    file << "Packet Size,Throughput (pps),Throughput (Bps),CPU Utilization (%),Jitter,Packets "
            "Lost,Packets "
            "Sent\n";
    for (const auto& result : results)
    {
        file << result.packetSize << "," << result.throughput << "," << result.throughputBytes
             << "," << result.cpuUtilization << "," << result.jitter << "," << result.packetsLost
             << "," << result.packetsSent << "\n";
    }
    file.close();
}
...
main(int argc, char* argv[])
{
    ...
    uint32_t nPackets = 10;               // Número de pacotes para cada teste
